      ...
      <Modal open={open && !showAuth} onClose={onClose} size="sm">
        ...
        <ModalBody>
          <div className={styles.content}>
            <p>Anda akan melakukan <strong>{isApprove ? 'Approve' : 'Reject'}</strong> pada item berikut:</p>
            
            <div className={styles.infoBox}>
              <div className={styles.infoRow}><span>Barcode:</span> <code>{item.barcode}</code></div>
              <div className={styles.infoRow}><span>Tahap:</span> {TAHAP_LABEL[item.tahap]}</div>
              <div className={styles.infoRow}><span>QTY Target:</span> {item.qtyTarget} pcs</div>
              <div className={styles.infoRow}><span>QTY Aktual:</span> <strong>{item.qtyAktual} pcs</strong></div>
            </div>

            <div className={styles.alert} data-type={mode}>
              {isApprove ? (
                <p>✅ <strong>APPROVE</strong>: QTY Aktual ({item.qtyAktual} pcs) akan dipertahankan. Bundel akan dibuka kuncinya untuk tahap berikutnya.</p>
              ) : (
                <p>❌ <strong>REJECT</strong>: QTY akan dikembalikan ke Target ({item.qtyTarget} pcs). Bundel akan dibuka kuncinya untuk tahap berikutnya.</p>
              )}
            </div>
